...
write_results = int(sys.argv[2]) if len(sys.argv) > 2 else 0
...
err_hist = np.zeros(timesteps)
_, err_hist = complete_run_neurofem_history(A, b, npm, timesteps)

if write_results:
    print("Finished computations, writing output")
    with open(f"{problem_name}_theoreticalneurofem_npm{npm}_history.csv", "w") as f:
        print("timestep;error_norm", file=f)
...
print("Finished computations, writing output")
with open(f"{problem_name}_theoreticalneurofem_multinpm.csv", "w") as f:
